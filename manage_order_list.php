...
$data_qry="SELECT id,order_id,user_id,menu_id,total_price FROM tbl_payment ORDER BY id";
...
{   
    ...
    function get_order_status($order_id)
    {
          global $mysqli;
    		...
    		$query1="SELECT * FROM tbl_payment WHERE tbl_payment.order_id='".$order_id."'";
					...
					<table class="table table-bordered table-hover datatable-highlight">
						...
						<?php 
							...
							<tr>
                                ...
                                <td>
                                    <div class="btn-group">
                                        <button type="button" class="btn <?php if(get_order_status($row['order_id'])=="Ready For Pickup"){?>btn bg-teal  <?php }else if(get_order_status($row['order_id'])=="Completed"){?> btn-success   <?php }else{?>btn-danger<?php }?> dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><?php echo get_order_status($row['order_id']);?></button>
                                        <div class="dropdown-menu" x-placement="top-start">
                                            ...
                                            <li><a class="dropdown" href="manage_order_list.php?status_pending_id=<?php echo $row['order_id'];?>&status_value=Could Not Be Made">Could Not Be Made</a> </li>
								...
								<td class="text-center">
									<ul class="icons-list">
                                                <li><a href="manage_order_list_view.php?order_id=<?php echo $row['order_id'];?>"> <i class="icon-eye2">&emsp;</i></a></li> 
                                                <li><a href="?order_id=<?php echo $row['order_id'];?>" title="Delete" onclick="return confirm('Are you sure you want to delete this category?');"><i class="glyphicon glyphicon-trash"></i></a></li>
                                                <br><a> View &emsp; Delete </a>
