...
else if(isset($_REQUEST['sub_menu_list']))
{		  
...
else if(isset($_GET['slider_list']))
{
		...
		while($data = mysqli_fetch_assoc($sql))
		{
			...
			$reponseobj['response']->message = 'Retrived Data From OSO';
...
else if(isset($_REQUEST['api_cart_add_update']))
{
 	 ...
 	 {		
		 ...
		 if($_REQUEST['menu_qty'] == 0)
		 {
	        ...
	        $cart_rowcount=mysqli_num_rows($cart_sql);
			 ...
			 $set['response']->message = 'Item Deleted';
		 ...
		 else
		 {
			...
			$set['response']->message = 'Basket Updated';
			$set['response']->code = 200;
			$set['add_cart']=array('cart_items'=>$cart_rowcount,'success'=>'1');	 
		 }
 	 }
 	 ...
 	 else
 	 {
			...
			$set['response']->message = 'Item Added To Basket';
			$set['response']->code = 200;
			$set['add_cart']=array('cart_items'=>$cart_rowcount,'success'=>'1');	
...
{		  
			...
			while($data = mysqli_fetch_assoc($sql1))
		{
			...
			$set['response']->message = 'Retrived Data From OSO';
			$set['response']->code = 200; 
			...
			else
			{
				...
				$set['response']->message = 'No Comments Yet';
...
else if(isset($_GET['promo_code']))
{
		...
		while($data = mysqli_fetch_assoc($sql))
		{
			...
			$reponseobj['message'] = 'Retrived Data From OSO';
			$reponseobj['code'] = 200;    					
...
{      
	...
	{ 
		if($data['status'] == 1)
		{
			if ($data['promo_start_date'] <= $today_date && $data['promo_end_date'] >= $today_date) 
			{
				...
				$querydd = mysqli_query($mysqli,"select SUM(menu_total_price)as total from tbl_order_items where order_id = '".$_GET['order_id']."'");
					...
					{
						$reponseobj['message'] = 'Apply Coupon';
						$reponseobj['code'] = 200; 
					}
					else
					{
						
						$reponseobj['message'] = 'No Coupon';
						$reponseobj['code'] = 404;    	
					}
					...
					$reponseobj['message'] = 'Applied A Promo';
					$reponseobj['code'] = 200;    					
		...
		{
    		$reponseobj['message'] = 'Promo Is Not Currently Active';
			$reponseobj['code'] = 404;    					
		}
	}
	else
	{
			$reponseobj['message'] = 'Promo Is Not Available';
			$reponseobj['code'] = 404;
	}
...
else if(isset($_GET['comment']))
{
	...
	$set['response']->message = 'You Just Posted A Comment';
	$set['response']->code = 200; 
...
//start search
  ...
  {	
		...
		{
				$set['message'] = 'No Dish Found! Please Try A Different Keyword';
				$set['code'] = 404;
...
function get_order_unique_id()
{
		...
		foreach($menu as $id)
		{
			...
			$sql = "INSERT INTO tbl_payment(order_id,cart_id, user_id, menu_id, total_price, payment_id,payment_type) values 
							('".$order_id."','".$cart_id."', '".$user_id."', '".$menu_id."','".$total_price."','".$payment_id."','".$payment_type."')";
				  mysqli_query($mysqli,$sql) or die(mysqli_error($mysqli));
		
				
				  
   				$set['message'] = 'Payment Is Successful' ;
				$set['code'] = 200;
...
else if(isset($_REQUEST['payment_detail_ios']))
{
		...
		foreach($menu as $id)
		{
			...
			while($data = mysqli_fetch_assoc($sql))
			{
				...
				$sql = "INSERT INTO tbl_payment(order_id,cart_id, user_id, menu_id, total_price, payment_id) values 
							('".$order_id."','".$cart_id."', '".$user_id."', '".$menu_id."','".$total_price."','".$payment_id."')";
				  mysqli_query($mysqli,$sql) or die(mysqli_error($mysqli));
